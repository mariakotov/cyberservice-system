...
![CyberService UML Diagram](./public/CyberService-Project.UML.png)
...
| UC ID | Name | Primary Actor | Supporting Actors |
|---|---|---|---|
| 001 | רישום עצמאי למערכת | לקוח | בסיס נתונים |
| 002 | רישום לקוח ע"י נציג | נציג שירות | לקוח, בסיס נתונים |
| 003 | הוספת משתמש פנימי | מנהל מערכת | בסיס נתונים |
| 004 | התחברות למערכת | משתמש מערכת | בסיס נתונים |
| 005 | שחזור סיסמה | משתמש מערכת | בסיס נתונים, מערכת התראות |
| 006 | עדכון פרטי חשבון לקוח | נציג שירות, לקוח | בסיס נתונים |
| 007 | ניהול הרשאות גישה | מנהל מערכת | בסיס נתונים |
| 008 | השבתת חשבון משתמש | מנהל מערכת | בסיס נתונים |
| 009 | צפייה בתיק לקוח אישי | מנהל צוות יועצים, יועץ סייבר, מנהל, נציג שירות, לקוח | בסיס נתונים |
| 010 | צפייה בפרטי התקשרות לקוח | יועץ סייבר, נציג שירות | בסיס נתונים |
| 011 | הורדת דוח פעילות תקופתי | לקוח, נציג שירות | בסיס נתונים, מערכת הפקת דוחות |
| 012 | צפייה ביומן פעולות מערכת | מנהל מערכת | בסיס נתונים |
| 013 | צפייה בקטלוג שירותים | משתמש מערכת | בסיס נתונים |
| 014 | הגדרת שירות סייבר חדש | מנהל מערכת | בסיס נתונים |
| 015 | עדכון פרטי ומחירי שירותים | מנהל כספים, מנהל מערכת | בסיס נתונים |
| 016 | השבתת שירות מהקטלוג | מנהל מערכת | בסיס נתונים |
| 017 | הזמנת שירות סייבר | נציג שירות, לקוח | בסיס נתונים, מערכת תשלומים |
| 018 | הצטרפות למסלול מנוי | נציג שירות, לקוח | בסיס נתונים, מערכת תשלומים |
| 019 | ביטול הזמנה או מנוי | נציג שירות, לקוח | בסיס נתונים, מערכת התראות, מערכת תשלומים |
| 020 | שינוי מועד ביצוע שירות | יועץ סייבר, לקוח, נציג שירות | בסיס נתונים, מערכת התראות |
| 021 | צפייה בלוח הזמנות פעילות | מנהל צוות יועצים, יועץ סייבר, נציג שירות | בסיס נתונים |
| 022 | מעקב אחר סטטוס ביצוע | יועץ סייבר, לקוח, נציג שירות | בסיס נתונים, מערכת התראות |
| 023 | הקצאת יועץ למשימה | מנהל צוות יועצים | בסיס נתונים, יועץ סייבר |
| 024 | הקצאה מחדש של יועץ | נציג שירות, מנהל צוות יועצים | בסיס נתונים, מערכת התראות |
| 025 | קביעת עדיפות למשימה | מנהל צוות יועצים | בסיס נתונים |
| 026 | אישור קבלת משימה | מנהל צוות יועצים, יועץ סייבר | בסיס נתונים |
| 027 | עדכון התקדמות וסטטוס | מנהל צוות יועצים, יועץ סייבר | בסיס נתונים, מערכת התראות |
| 028 | ניהול יומן פגישות יועץ | לקוח, יועץ סייבר | בסיס נתונים, מערכת התראות |
| 029 | הזנת ממצאי בדיקה | יועץ סייבר | בסיס נתונים, מערכת הפקת דוחות |
| 030 | העלאת מסמכי תמיכה טכניים | יועץ סייבר | בסיס נתונים |
| 031 | תיעוד שעות עבודה | מנהל צוות יועצים, יועץ סייבר | בסיס נתונים, מנהל כספים |
| 032 | ניהול הרשאות למערכות לקוח | יועץ סייבר | לקוח, בסיס נתונים |
| 033 | יצירת טיוטת דוח סיכונים | יועץ סייבר | בסיס נתונים, מערכת הפקת דוחות |
| 034 | אישור דוח סיכונים סופי | יועץ סייבר, מנהל צוות יועצים | בסיס נתונים |
| 035 | פרסום דוח סופי לקוח | לקוח, מנהל צוות יועצים | בסיס נתונים, מערכת התראות |
| 036 | שליחת התראה על ממצא קריטי | מנהל צוות יועצים, לקוח, יועץ סייבר | בסיס נתונים, מערכת התראות |
| 037 | ביצוע תשלום | נציג שירות, לקוח | בסיס נתונים, מערכת תשלומים |
| 038 | ניהול חיובי מנויים | מערכת תשלומים | בסיס נתונים, לקוח |
| 039 | הפקת קבלה וחשבונית | מנהל כספים, לקוח, מערכת תשלומים | בסיס נתונים, מערכת התראות |
| 040 | ביצוע החזר כספי | לקוח, מנהל כספים | בסיס נתונים, מערכת התראות, מערכת תשלומים |
| 041 | צפייה בפירוט חיובי עבר | מנהל כספים, נציג שירות, לקוח | בסיס נתונים, מערכת תשלומים |
| 042 | שליחת פנייה למוקד שירות | נציג שירות, לקוח | בסיס נתונים, מערכת התראות |
| 043 | טיפול בקריאת שירות | לקוח, יועץ סייבר, נציג שירות | בסיס נתונים, מערכת התראות |
| 044 | מתן משוב ודירוג שירות | מנהל צוות יועצים, לקוח | בסיס נתונים |
| 045 | הפקת דוח עומס יועצים | מנהל צוות יועצים | בסיס נתונים, מערכת הפקת דוחות |
| 046 | צפייה בלוח בקרה ניהולי | מנהל מערכת, מנהל כספים, מנהל צוות יועצים | בסיס נתונים |
| 047 | הפקת דוח מגמות תקיפה | מנהל מערכת, מנהל צוות יועצים | בסיס נתונים, מערכת הפקת דוחות |
| 048 | הכנסת נתונים למערכת | בסיס נתונים | אין |
| 049 | שליפת נתונים מהמערכת | בסיס נתונים | אין |
| 050 | עדכון נתונים במערכת | בסיס נתונים | אין |
